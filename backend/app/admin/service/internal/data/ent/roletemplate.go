...
type RoleTemplate struct {
	...
	// 状态
	Status *roletemplate.Status `json:"status,omitempty"`
	// 排序值（越小越靠前）
...
func (*RoleTemplate) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case roletemplate.FieldPermissions:
			values[i] = new([]byte)
		...
		case roletemplate.FieldCreatedAt, roletemplate.FieldUpdatedAt, roletemplate.FieldDeletedAt:
...
func (_m *RoleTemplate) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case roletemplate.FieldDeletedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
		...
		case roletemplate.FieldCreatedBy:
		...
		case roletemplate.FieldUpdatedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
		...
		case roletemplate.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Status = new(roletemplate.Status)
				*_m.Status = roletemplate.Status(value.String)
			}
		...
		case roletemplate.FieldCategory:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case roletemplate.FieldIsDefault:
		...
		case roletemplate.FieldIsSystem:
			if value, ok := values[i].(*sql.NullBool); !ok {
