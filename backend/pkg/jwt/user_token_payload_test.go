...
func TestNewUserTokenPayload(t *testing.T) {
	...
	payload := NewUserTokenPayload(&user, &ds, &ou, &client, &device, &isP, &isT)
...
func TestNewUserTokenAuthClaims(t *testing.T) {
	...
	claims := NewUserTokenAuthClaims(&user, &ds, &ou, &client, &device, &isP, &isT)
	...
	assert.Equal(t, user.Roles, (*claims)[ClaimFieldRoleCodes])
