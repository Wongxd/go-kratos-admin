import {enUSCore, type EnUSCore} from './_core';
import {enUSModules, parseModulePath, loadModule} from './_modules';
...
// 类型导出（核心完美类型 + 扩展宽松类型）
export type EnUSResources = EnUSCore & {
    // 扩展模块：运行时加载，类型宽松
...
export const getAvailableModules = async (): Promise<string[]> => {
    ...
    for (const path of Object.keys(enUSModules)) {
