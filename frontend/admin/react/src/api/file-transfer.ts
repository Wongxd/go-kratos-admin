import {requestApi, requestClient} from "@/core";
import {createFileTransferServiceClient} from "@/api/generated/admin/service/v1";

const fileTransferServiceClient = createFileTransferServiceClient(requestApi);
 ...
 */
export async function downloadFile(bucketName: string, objectName: string, preferPresignedUrl: boolean) {
    if (preferPresignedUrl) {
        const resp = await fileTransferServiceClient.DownloadFile({
