...
export const createAccessibleRouter = async (options: GenerateMenuAndRoutesOptions) => {
    ...
    if (options.autoInjectRedirect !== false) routes = injectRedirects(routes as unknown as AppRoute[]) as unknown as AppRouteObject[];
    if (options.autoSort !== false) routes = sortRoutes(routes as unknown as AppRoute[]) as unknown as AppRouteObject[];
