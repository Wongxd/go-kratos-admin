<template>
  ...
  >
    ...
    >
      ...
      <div v-if="isActive('table')" class="toolbar-group">
        <button
          type="button"
          class="toolbar-btn"
...
import { ElDialog, ElInput, ElSelect, ElOption, ElSwitch, ElMessageBox } from "element-plus";
...
const toolbarActions = {
  ...
  toggleHeading: (level: Level) => editor.value?.chain().focus().toggleHeading({ level }).run(),
  ...
  toggleSuperscript: () => editor.value?.chain().focus().toggleSuperscript().run(),
  setParagraph: () => editor.value?.chain().focus().setParagraph().run(),
  clearFormatting: () => editor.value?.chain().focus().unsetAllMarks().clearNodes().run(),
  ...
  toggleHeaderCell: () => editor.value?.chain().focus().toggleHeaderCell().run(),
  setAlign: (align: "center" | "justify" | "left" | "right") =>
    editor.value?.chain().focus().setTextAlign(align).run(),
  setTextColor: (color: string) => editor.value?.chain().focus().setColor(color).run(),
  setHighlight: (color: string) => editor.value?.chain().focus().toggleHighlight({ color }).run(),
...
.tiptap-editor-wrapper :deep(.ProseMirror pre .code-block-language-selector select) {
