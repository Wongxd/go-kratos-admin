...
import { createUserProfileServiceClient } from "@/api/generated/admin/service/v1";
...
export const useUserProfileStore = defineStore("user-profile", () => {
   ...
   */
  async function verifyPhone(phone: string, code: string, verificationId?: string) {
   ...
   * @param verificationId 服务端生成的验证码会话ID
   */
  async function verifyEmail(email: string, code: string, verificationId?: string) {
    return await userProfileService.VerifyContact({
      email: { email, code },
      verificationId,
    });
